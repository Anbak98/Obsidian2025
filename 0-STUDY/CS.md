...
- Unity 스크립트는 사전에 정해진 순서에 따라 이벤트 함수들이 호출되며 실행됩니다. 이 과정을 Unity 생명주기라고 하며, 대표적으로 `Awake()`, `Start()`, `Update()`, `LateUpdate()` 같은 함수들이 해당 순서에 맞게 자동으로 실행됩니다. 이 순서를 이해하면 컴포넌트 간 의존성 처리나 최적화에 도움이 됩니다.
